...
class TripleMA(Indicator):
    ...
    def trend(self):
        ...
        uptrend = last_price["SMA50"].item() > last_price["SMA100"].item() and last_price["SMA100"].item() > last_price["SMA200"].item()
        downtrend = last_price["SMA50"].item() < last_price["SMA100"].item() and last_price["SMA100"].item() > last_price["SMA200"].item()
        self.price = last_price["Close"].item()
        buy = self.price > last_price["SMA200"].item() and self.price < last_price["SMA50"].item() 
        sell = self.price < last_price["SMA200"].item() and self.price > last_price["SMA50"].item()
        if uptrend:
            self.result = "uptrend"
        if downtrend: 
